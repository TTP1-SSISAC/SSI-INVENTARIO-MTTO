...
<td>${k['Usuario'] || '—'}</td>
</tr>`).join('') : `<tr class="empty-row"><td colspan="9">Sin movimientos registrados.</td></tr>`;
}
...
// Métodos de reparación predeterminados (esto NO cambia: sigue siendo
// solo local, ampliable con el botón + sin persistir en ninguna hoja).
...
// Repuebla el select "Nombre de la reparación" desde state.reparaciones
// (hoja BD_REPARACIONES, columna "Nombre"), conservando la selección
// actual si sigue existiendo en la lista tras recargar.
function renderNombreReparacionSelect() {
const selNombre = document.getElementById('repNombre');
if (!selNombre) return;
const actual = selNombre.value;
const nombres = state.reparaciones.map(r => r['Nombre']).filter(Boolean);
selNombre.innerHTML = nombres.length
? nombres.map(n => `<option value="${n}">${n}</option>`).join('')
: '<option value="" disabled selected>— Sin reparaciones registradas —</option>';
if (actual && nombres.includes(actual)) selNombre.value = actual;
}

// Agrega un nuevo nombre de reparación a la hoja BD_REPARACIONES vía
// backend (acción 'addNombreReparacion') y refresca el select.
async function addNombreReparacion() {
const val = window.prompt('Nombre de la nueva reparación:');
if (!val || !val.trim()) return;
const nombre = val.trim();
try {
await apiPost('addNombreReparacion', { 'Nombre': nombre });
toast('Reparación agregada al catálogo', 'ok');
await loadAll();
const selNombre = document.getElementById('repNombre');
if (selNombre) selNombre.value = nombre;
} catch (err) {
...
selProd.innerHTML = '<option value="" disabled selected>— Selecciona producto —</option>' +
state.productos.map(p =>
`<option value="${p['Código Producto']}">${p['Código Producto']} — ${p['Producto']} (stock: ${getStockDe(p['Código Producto'])})</option>`
).join('');
